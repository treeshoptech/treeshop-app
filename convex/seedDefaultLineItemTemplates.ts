...
export const seedDefaults = mutation({
  handler: async (ctx) => {
    ...
    await ctx.db.insert("lineItemTemplates", {
      ...
      notes: "Uses MulchingScore formula: DBH Package (inches) × Acreage × AFISS Multiplier. Production rate varies by equipment (1.3-5.0 PpH typical).",
      usageCount: 0,
      createdAt: now,
      updatedAt: now,
    });
    ...
    await ctx.db.insert("lineItemTemplates", {
      ...
      notes: "Uses TreeScore formula: Height × Crown Radius × 2 × DBH ÷ 12. AFISS multipliers applied per tree based on access method, proximity to structures, and tree condition. Production rate: ~250 PpH (varies by crew/equipment).",
      usageCount: 0,
      createdAt: now,
      updatedAt: now,
    });
    ...
    await ctx.db.insert("lineItemTemplates", {
      ...
      notes: "Uses TrimScore formula: TreeScore base × Trim Factor. Light (10-15%) ×0.3, Medium (20-30%) ×0.5, Heavy (40-50%) ×0.8. Production rate: ~300 PpH (varies by trim type).",
      usageCount: 0,
      createdAt: now,
      updatedAt: now,
    });
  ...
  handler: async (ctx) => {
    ...
    const templates = [
      ...
      },
      {
        ...
        productionTaskType: "Production",
        defaultUnit: "Each",
        afissFactorIds: ["irregularities_hardwood", "irregularities_large_root_flare", "irregularities_rotten_stump", "site_rocky_ground", "facilities_tight_landscaping", "access_narrow_gate", "access_soft_ground"],
        tags: ["stump-grinding", "stump-removal", "grinding"],
        notes: "Uses StumpScore formula: Diameter² × (Height Above + Grind Depth Below). Modifiers: Hardwood +15%, Large root flare +20%, Rotten -15%. Production rate: 400 StumpScore points per hour (default).",
      },
      {
        ...
        productionTaskType: "Production",
        defaultUnit: "Tree",
        afissFactorIds: ["irregularities_dead_hazard", "irregularities_leaning_tree", "irregularities_multi_trunk", "facilities_power_lines_touching", "facilities_power_lines_nearby", "facilities_buildings_within_50ft", "facilities_pool_high_value", "access_no_bucket", "safety_high_voltage", "safety_confined_space", "safety_near_public_road"],
        tags: ["tree-removal", "felling", "hazard-tree"],
        notes: "Uses TreeScore formula: Height × Crown Radius × 2 × DBH ÷ 12. AFISS multipliers applied per tree based on access method, proximity to structures, and tree condition. Production rate: ~250 PpH (varies by crew/equipment).",
      },
      {
        ...
        productionTaskType: "Production",
        defaultUnit: "Tree",
        afissFactorIds: ["facilities_power_lines_touching", "facilities_power_lines_nearby", "facilities_buildings_within_50ft", "facilities_pool_high_value", "access_no_bucket", "irregularities_dead_hazard", "safety_high_voltage", "safety_confined_space"],
        tags: ["tree-trimming", "pruning", "crown-reduction"],
        notes: "Uses TrimScore formula: TreeScore base × Trim Factor. Light (10-15%) ×0.3, Medium (20-30%) ×0.5, Heavy (40-50%) ×0.8. Production rate: ~300 PpH (varies by trim type).",
      },
      {
        ...
        productionTaskType: "Production",
        defaultUnit: "Day",
        afissFactorIds: ["access_soft_ground", "access_steep_slope", "facilities_utilities_in_zone", "facilities_buildings_within_50ft", "facilities_power_lines_nearby", "site_wetlands", "site_rocky_ground", "site_dense_undergrowth", "site_protected_habitat", "irregularities_dead_hazard"],
        tags: ["land-clearing", "site-prep", "excavation"],
        notes: "Day-based pricing: Standard Lot (1 day), Large Lot (2 days), Multi-Lot (3+ days). Intensity levels: Light, Standard, Heavy. AFISS factors add 0.5-1 day. Daily rate = 8 hours × Loadout Hourly Rate.",
      },
    ];
